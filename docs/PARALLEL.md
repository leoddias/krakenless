...
How several agents work on Krakenless at the same time without corrupting
each other's work, and how each unit of work is driven to *done* (built,
tested, reviewed) instead of *written*.
...
| Reviewer | `conventions-reviewer`, `safety-reviewer` agents | read-only verdicts inside the loop |
...
- **Review:** conventions | conventions+safety (safety required if it touches
  `src/git/**` or `src-tauri/src/git_runner.rs`)
...
1. **Disjoint ownership.** Two active packets must not share an owned glob.
   Shared files (`package.json`, `src/state/**`, barrel/index files, ROADMAP
   checkboxes) belong to the orchestrator — a worker requests the edit in its
   report instead of making it.
2. **Vertical, not layered.** Prefer "parser + its tests + its types" over
   "all parsers" / "all tests". A packet that can't be tested alone is too thin.
...
3 test     run the suite; parsers/builders must ship tests in the same change
4 review   conventions-reviewer (+ safety-reviewer when the packet says so)
...
2. Run the **full** suite — not just the packet's tests.
...
After all packets merge: full suite once more, then `/handoff`.
