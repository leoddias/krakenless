...
1. **State check.** Read `docs/PROGRESS.md` (Current state, Next up) and the
   active milestone in `docs/ROADMAP.md`. Confirm the suite is green and the
   working tree is clean — never fan out from a red or dirty base.
...
3. **Settle contracts first.** If packets share a type or signature, write it
   (types only, no implementations), test-nothing, and commit it before
   spawning. Workers treat it as read-only.
4. **Write the packets** into `docs/TASKS.md` using the packet format:
   ...
   (`conventions+safety` for anything under `src/git/**` or
   `src-tauri/src/git_runner.rs`), status `queued`.
...
5. **Spawn** one `task-worker` per packet, all in a single message so they run
   ...
   edits must be *requested*, not made.
...
7. **Integrate one at a time** (dependency order): rebase the worker's branch
   on `main`, run the full suite, verify the diff stayed inside owned globs,
...
8. **Close.** Full suite once more, clear `docs/TASKS.md` § In flight, then run
   `/handoff`. Record any packet collision in the session log as a splitting
   lesson.
